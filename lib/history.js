...
const latestImagePath = path.resolve(historyPath, "latest.jpg");
...
async function store(image, date) {
  const imagePath = path.resolve(historyPath, "" + date.getTime()) + ".jpg";
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (!fullName.endsWith(".jpg")) continue;

    const name = fullName.substring(0, fullName.length - 4);
    // "simple assumption" that the name is a number, e.g original as 1231313123233.jpg
